	   ...
	   stage('Build')
	   {
	      steps
	      {
        	sh ''' 
		  	...
		  	ssh -o "StrictHostKeyChecking no" boaz@$AZURE_IP 'sudo -i && cp   /home/boaz/.ssh/authorized_keys /root/.ssh/authorized_keys
		  '''
		sh 'ansible-playbook -i inventory.yaml play.yaml -b --become-user=root'    
	      }
	   }
	   stage('Test') 
	   {		  
