  ...
  { 
	...
	stages 
	{
	   ...
	   stage('Build')
	   {
		...
		sh 'export azure_ip=$AZURE_IP && sed -i "s/AZURE_IP/$azure_ip/g" inventory.yaml && ansible-playbook -i playbooks/inventory.yaml azure-deploy-minkube-playbook.yaml '//-b --become-user=root    
	      }
	   }
	   stage('Test') 
	   {		  
